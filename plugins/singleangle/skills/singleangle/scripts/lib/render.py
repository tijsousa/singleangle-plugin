"""Output rendering for singleangle skill."""
...
def render_compact(report: schema.Report, limit: int = 15, missing_keys: str = "none") -> str:
    ...
    if report.mode == "web-only":
        ...
        lines.append("- Edit `~/.config/singleangle/.env` to add keys")
        lines.append("---")
        lines.append("")
...
def write_outputs(
    ...
    # singleangle.context.md
    with open(OUTPUT_DIR / "singleangle.context.md", 'w') as f:
...
def get_context_path() -> str:
    ...
    return str(OUTPUT_DIR / "singleangle.context.md")
