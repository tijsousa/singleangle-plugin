"""Environment and API key management for singleangle skill."""

import os
from pathlib import Path
from typing import Optional, Dict, Any

CONFIG_DIR = Path.home() / ".config" / "singleangle"
...
def get_config() -> Dict[str, Any]:
    """Load configuration from ~/.config/singleangle/.env and environment."""
...
def validate_sources(requested: str, available: str, include_web: bool = False) -> tuple[str, Optional[str]]:
    ...
    if available == 'web':
        if requested == 'auto':
            ...
            return 'web', f"No API keys configured. Using WebSearch fallback. Add keys to ~/.config/singleangle/.env for Reddit/X."
